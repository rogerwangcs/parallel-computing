...
void __global__ transpose_kernel_read_and_write_coalesced_with_shared_memory(int *a, int *b, int n){
...
void __global__ transpose_kernel_read_and_write_coalesced_with_shared_memory_less_conflicts(int *a, int *b, int n){
	...
	__shared__ int tmpmat[(DIM+1)*(DIM+1)]; // 1D shared memory

	tmpmat[threadIdx.y + threadIdx.x*(DIM+1)] = b[r];

	__syncthreads(); // forces all threads inside the block to reach here. All memory transactions are visible as completed to other threads
					// no synchronism between threads in different blocks.

	a[s] = tmpmat[threadIdx.x + threadIdx.y*(DIM+1)];
...
int main(){
	...
	gstart();
	transpose_kernel_read_and_write_coalesced_with_shared_memory<<<numblockspergrid,numthreadsperblock>>>(h_d_a, h_d_b,  n);
	gend(&gpu_time);
	printf("GPU transpose read_and_write_coalesced_with_shared_memory time  = %f\n",gpu_time);

	gstart();
	transpose_kernel_read_and_write_coalesced_with_shared_memory_less_conflicts<<<numblockspergrid,numthreadsperblock>>>(h_d_a, h_d_b,  n);
	gend(&gpu_time);
	printf("GPU transpose read_and_write_coalesced_with_shared_memory_less_conflicts time  = %f\n",gpu_time);
